...
import {
    classSolutionsMock,
    solutionsMock,
} from '../const/solutionsConstMocks.ts';

export const solutionsHandlers = [
    http.get(
        `${appPaths.basePath}${appPaths.solution(':id')}`,
        ({ params }) => {
    ...
    http.get(
        `${appPaths.basePath}${appPaths.homeworkSolutions(':id')}`,
        ({ params }) => {
            const id = Number(params.id);
            ...
            try {
                return HttpResponse.json(
                    {
                        solutions: solutionsMock.filter((i) => i.hwID === id),
                    },
                    {
                        status: 200,
                        headers: { ...defaultHeadersMock },
