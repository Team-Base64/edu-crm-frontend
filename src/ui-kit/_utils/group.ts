...
export function groupByValues<Item extends Object, Key extends Extract<keyof Item, PropertyKey>>(items: Item[], keys: Key[]) {
            ...
            .stringify(
                Object.assign({},
                    ...keys
                        .map(
                            key => {
                                if (typeof item[key] === 'function') throw new Error(`Functions not allowed as keys. key is ${JSON.stringify(key)}`);
                                return { [key]: item[key] };
                            }
                        )
                )
...
export function groupByValuesObjectKeys<Item extends Object, Key extends Extract<keyof Item, PropertyKey>>(items: Item[], keys: Key[]) {
    const ret = new Map<{ [key in Key]: Extract<Item[Key], Item[key]> }, Item[]>();
