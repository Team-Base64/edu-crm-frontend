...
const AnnounceCreateField: React.FC<AnnounceCreateFieldProps> = ({ submit, avatarSrc }) => {
    const id = useId();
    ...
    const handleSubmitBtnClick = useCallback((e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        handleSubmit();
    }, [handleSubmit]);
    ...
    const handleKeydown = useCallback((e: KeyboardEvent) => {
        if(!textarea_ref.current || textarea_ref.current !== document.activeElement){
            return;
        }
    ...
    return (
        ...
        <Container classes={styles.card} direction="horizontal">
            ...
            <Icon
                ...
                classes={styles.btn} name='chatSend' />
        </Container>
    );
}
