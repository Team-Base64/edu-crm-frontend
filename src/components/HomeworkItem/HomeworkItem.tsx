...
const HomeworkItem: React.FC<HomeworkItemProps> = ({ title, description, deadlineTime, onClick, classes }) => {
    let stateClassName = styles.notPass;
    let stateStr = 'До ' + new Date(deadlineTime).toLocaleDateString('ru-RU').slice(0, -5);
    ...
    return (
        <Container classes={[styles.card, classes].join(' ')} direction="horizontal" onClick={onClick}>
            <Container classes={styles.wrapper} direction="vertical">
                <Text classes={[styles.text, styles.title].join(' ')} type="h6" weight="bold">{title}</Text>
                <Text classes={[styles.text, styles.description].join(' ')} type="p2" weight="regular">{description}</Text>
            </Container>
